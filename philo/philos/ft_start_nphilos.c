...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2022/11/26 16:46:44 by makacem           #+#    #+#             */
/*   Updated: 2022/12/05 16:44:15 by makacem          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../philo.h"

void	*nroutine(void *arg)
{
	t_table	*temp;
	int		index;
	t_fork	fork;
	...
	while (++fork.n_eat <= temp->arguments.nbr_tms_pilos_eat && temp->dead == 0)
	{
		fork.ttd = fork.ttd;
...
void	ft_start_nphilos(t_table *table)
{
	...
	{
		pthread_create(&(table->philos->philosopher), NULL, &nroutine, table);
		usleep(1000);
		table->philos = table->philos->next;
	}
